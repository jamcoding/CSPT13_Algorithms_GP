...
def naive_scavenging(field):
    ...
    for perm in list_of_perms:
        total_gold = field[0][0] # how we will access the field in second list notation
        col = 0
        row = 0
        ...
        # after getting to the edge of the field, check if this path had more gold than previous paths
    ...
    for col in range(field_length - 1, -1, -1):
        for row in range(field_length):
            ...
            if col != field_length - 1:
                e = gold_cache[row][col + 1]
            # look at gold collected if we chose northeast
            if row != 0 and col != field_length - 1:
                ne = gold_cache[row - 1][col + 1]
            # look at gold collected if we chose southeast
            if row != field_length - 1 and col != field_length - 1:
                se = gold_cache[row + 1][col + 1]

            # update the cache with how much gold we collected from choosing the path that will lead to the most gold + the gold from the location we currently stand in
            # could use a max function but we are keeping track of the path that we follow to find the gold

            if e > ne and e > se:
                gold_cache[row][col] += field[row][col] + e
                if col < field_length - 1:
                    path_cache[row][col] += "e," + path_cache[row][col + 1]
            elif ne > e and ne > se:
                gold_cache[row][col] += field[row][col] + ne
                if col < field_length - 1:
                    path_cache[row][col] += "ne," + path_cache[row - 1][col + 1]
            else:
                gold_cache[row][col] += field[row][col] + se
                if col < field_length - 1 and row < field_length - 1:
                    path_cache[row][col] += "se," + path_cache[row + 1][col + 1]

    # because we are required to start in the northwest corner, the max gold collected will be the val and the path start at [0][0]

    num_gold = gold_cache[0][0]
    path = path_cache[0][0].split(",")
    path.pop()

    return f"{num_gold} can be acquired by moving {path}"
